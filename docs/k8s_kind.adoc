...
----
$ kind create cluster --config=kind/kind-1-cluster.yaml
Creating cluster "kind-1-cluster" ...
...
Set kubectl context to "kind-kind-1-cluster"
You can now use your cluster with:

kubectl cluster-info --context kind-kind-1-cluster
...
guest@DESKTOP-E39V4F4:~/tmp_github/kubernetes-examples$ kubectl create service nodeport nginx --tcp=80:80 --node-port=30000
